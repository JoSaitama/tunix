...
TUNIX_ROLLOUT_DIAGNOSTIC_PATH="${TUNIX_ROLLOUT_DIAGNOSTIC_PATH:-}"
TUNIX_ROLLOUT_DIAGNOSTIC_GROUPS="${TUNIX_ROLLOUT_DIAGNOSTIC_GROUPS:-}"
TUNIX_RESUME_ACTOR_CHECKPOINT_ROOT="${TUNIX_RESUME_ACTOR_CHECKPOINT_ROOT:-}"
TUNIX_RESUME_CHECKPOINT_STEP="${TUNIX_RESUME_CHECKPOINT_STEP:-}"
...
TUNIX_TRAINING_SUMMARY_PATH="${TUNIX_TRAINING_SUMMARY_PATH:-}"
...
TUNIX_VLLM_CLEAR_JAX_CACHES="${TUNIX_VLLM_CLEAR_JAX_CACHES:-true}"
TUNIX_VLLM_KV_CACHE_REFRESH_MODE="${TUNIX_VLLM_KV_CACHE_REFRESH_MODE:-reuse}"
TUNIX_VLLM_STUCK_STACK_DUMP_SECONDS="${TUNIX_VLLM_STUCK_STACK_DUMP_SECONDS:-300}"
...
# These are validated routing candidates. grpo_main_distributed exchanges the
# local addresses after JAX starts and rewrites them into the TPU backend's
# actual process-rank order before distributed-vLLM creates its transport.
...
echo "  TRAINING_SUMMARY_PATH=${TUNIX_TRAINING_SUMMARY_PATH:-<disabled>}"
...
run_worker() (
  ...
  export TUNIX_TRAINING_SUMMARY_PATH
  export TUNIX_POLICY_DTV_SCORE_BACKEND
  export TUNIX_DISTRIBUTED_ROLLOUT_PORT
  export TUNIX_VLLM_CLEAR_JAX_CACHES
  export TUNIX_VLLM_KV_CACHE_REFRESH_MODE
  export TUNIX_VLLM_STUCK_STACK_DUMP_SECONDS
  export TUNIX_RESUME_ACTOR_CHECKPOINT_ROOT TUNIX_RESUME_CHECKPOINT_STEP
  ...
  if [[ -n "${TUNIX_ROLLOUT_DIAGNOSTIC_PATH}" ]]; then
    export TUNIX_ROLLOUT_DIAGNOSTIC_PATH
  fi
  if [[ -n "${TUNIX_ROLLOUT_DIAGNOSTIC_GROUPS}" ]]; then
    export TUNIX_ROLLOUT_DIAGNOSTIC_GROUPS
  fi
  status=0
  set +e
...
export TUNIX_TRAINING_SUMMARY_PATH=${TUNIX_TRAINING_SUMMARY_PATH@Q}
...
export TUNIX_VLLM_CLEAR_JAX_CACHES=${TUNIX_VLLM_CLEAR_JAX_CACHES@Q}
export TUNIX_VLLM_KV_CACHE_REFRESH_MODE=${TUNIX_VLLM_KV_CACHE_REFRESH_MODE@Q}
export TUNIX_VLLM_STUCK_STACK_DUMP_SECONDS=${TUNIX_VLLM_STUCK_STACK_DUMP_SECONDS@Q}
export TUNIX_RESUME_ACTOR_CHECKPOINT_ROOT=${TUNIX_RESUME_ACTOR_CHECKPOINT_ROOT@Q}
export TUNIX_RESUME_CHECKPOINT_STEP=${TUNIX_RESUME_CHECKPOINT_STEP@Q}
...
if [[ -n ${TUNIX_ROLLOUT_DIAGNOSTIC_PATH@Q} ]]; then
  export TUNIX_ROLLOUT_DIAGNOSTIC_PATH=${TUNIX_ROLLOUT_DIAGNOSTIC_PATH@Q}
fi
if [[ -n ${TUNIX_ROLLOUT_DIAGNOSTIC_GROUPS@Q} ]]; then
  export TUNIX_ROLLOUT_DIAGNOSTIC_GROUPS=${TUNIX_ROLLOUT_DIAGNOSTIC_GROUPS@Q}
fi
status=0
set +e
