...
RUN_GLOB = os.environ.get("DPO_EVAL_RUN_GLOB", "dpo_qwen2p5_*_global_no_pref*_lora_full_*")
RUN_RE = re.compile(
    r"dpo_qwen2p5_"
    r"(?P<variant>.+)_(?P<corruption>clean|global_no_pref10|global_no_pref20|global_no_pref40|global_mismatch\d+)_lora_full_(?P<run_ts>\d{8}_\d{6})"
)
METHOD_ORDER = {
    "vanilla_dpo": 0,
    "self_inf": 1,
...
# RANDOM_NO_PREF_METHOD_PATCH
# Add random filtering variants used by no-pref baselines.
...
for _name in ["METHOD_DISPLAY_NAMES", "DISPLAY_NAMES", "METHOD_NAMES"]:
    if _name in globals():
        globals()[_name].update({
            "random_pair_filtering_filt5": "Random Pair Filtering (5%)",
            "random_pair_filtering_filt10": "Random Pair Filtering (10%)",
        })


# FIX_RANDOM_SPACE_NORMALIZED_ALIASES
# _normalize_method() converts "_" and "-" to spaces, so these aliases are required.
METHOD_ALIASES.update({
    "random pair filtering filt5": "random_pair_filtering_filt5",
    "random pair filtering 5": "random_pair_filtering_filt5",
    "random pair filtering 5pct": "random_pair_filtering_filt5",
    "random pair filtering filt10": "random_pair_filtering_filt10",
    "random pair filtering 10": "random_pair_filtering_filt10",
    "random pair filtering 10pct": "random_pair_filtering_filt10",
})


# FIX_REWARD_BASED_SPACE_NORMALIZED_ALIASES
# _normalize_method() converts "_" and "-" to spaces.
METHOD_ALIASES.update({
...
METHOD_ORDER.update({
    "self_inf_norm": METHOD_ORDER.get("self_inf", 1) + 0.05,
    "self_inf_loo": METHOD_ORDER.get("self_inf", 1) + 0.1,
    "self_inf_re_loo": 1.15,
    "self_inf_loo_cos": METHOD_ORDER.get("self_inf", 1) + 0.2,
})
...
def _discover_clean_runs(
  ...
  candidates = []
  runs_root = Path(os.environ.get("DPO_EVAL_RUNS_ROOT", str(repo_root / "runs"))).resolve()
  print(f"Discovering runs under {runs_root} with glob {RUN_GLOB}")
...
# === DYNAMIC_FILTER40_RB2_PATCH_BEGIN ===
# Add budget-matched 40% fixed-ratio baselines for RewardBench v2 eval.
...
METHOD_ORDER.update({
    "random_pair_filtering_filt40": METHOD_ORDER.get("random_pair_filtering_filt10", 3) + 0.1,
    "reward_based_filtering_filt40": METHOD_ORDER.get("reward_based_filtering_filt10", 5) + 0.1,
})

for _name in ["METHOD_DISPLAY", "METHOD_DISPLAY_NAMES", "DISPLAY_NAMES", "METHOD_NAMES"]:
  if _name in globals():
    globals()[_name].update({
        "random_pair_filtering_filt40": "Random Pair Filtering (40%)",
        "reward_based_filtering_filt40": "Reward-based Filtering (40%)",
    })
# === DYNAMIC_FILTER40_RB2_PATCH_END ===
