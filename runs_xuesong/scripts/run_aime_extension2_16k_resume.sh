...
case "$TUNIX_VLLM_CLEAR_JAX_CACHES" in
  true|false) ;;
  *)
    echo "TUNIX_VLLM_CLEAR_JAX_CACHES must be true or false; got $TUNIX_VLLM_CLEAR_JAX_CACHES" >&2
    exit 2
    ;;
esac
TUNIX_VLLM_STUCK_STACK_DUMP_SECONDS="${TUNIX_VLLM_STUCK_STACK_DUMP_SECONDS:-300}"
[[ "$TUNIX_VLLM_STUCK_STACK_DUMP_SECONDS" =~ ^[0-9]+([.][0-9]+)?$ ]] || {
  echo "TUNIX_VLLM_STUCK_STACK_DUMP_SECONDS must be a nonnegative number" >&2
  exit 2
}
...
echo "TUNIX_VLLM_CLEAR_JAX_CACHES=$TUNIX_VLLM_CLEAR_JAX_CACHES"
echo "TUNIX_VLLM_STUCK_STACK_DUMP_SECONDS=$TUNIX_VLLM_STUCK_STACK_DUMP_SECONDS"
