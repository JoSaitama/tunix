...
script_path="${repo_root}/examples/deepscaler/run_deepscaler_disagg_v5p16_selfinf_group_officialish_8k.sh"
...
# Keep local TPU batch size at 128, but align the other requested 8K settings.
model_cache_dir="${MODEL_CACHE_DIR:-/home/eve/models/deepseek-r1-distill-qwen-1.5b}"

num_batches="${num_batches:-630}" \
bash "${repo_root}/examples/deepscaler/run_deepscaler_disagg_v5p16_1epoch.sh" \
  model_config.model_download_path="${model_cache_dir}" \
  actor_model_config.model_download_path="${model_cache_dir}" \
  reference_model_config.model_download_path="${model_cache_dir}" \
  rollout_model_config.model_download_path="${model_cache_dir}" \
