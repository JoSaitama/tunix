...
"""Tests for policy-score, full-update ordinary DTV configuration."""
...
class PolicySelfInfTrainerTest(absltest.TestCase):
  ...
  def test_train_step_scores_policy_but_updates_total_gradients(self):
    ...
    trainer = object.__new__(
        self_inf_policy_trainer.PolicySelfInfTrainer
    )
    ...
    loss, _ = trainer._train_step(
        ...
        optimizer,
        {
            "policy": jnp.asarray([2.0, -1.0, 0.0, 0.0]),
            "total": jnp.asarray([20.0, 30.0, 40.0, 50.0]),
        },
    )

    # The first sample has a negative LOO cross term (2 * -1), but ordinary
    # DTV retains its positive self term and therefore keeps it. The second
    # sample is anti-aligned with the full policy mean and is filtered. Total
    # gradients from samples 0, 2, and 3 are used for the full update.
    np.testing.assert_allclose(loss, 2.0 * 110.0 / 3.0)
    np.testing.assert_allclose(
        optimizer.grads["weight"].get_value(), 110.0 / 3.0
    )
  ...
  def test_policy_score_loss_setter_is_separate_from_update_loss(self):
    ...
    trainer.clear_jit_cache = lambda: None
