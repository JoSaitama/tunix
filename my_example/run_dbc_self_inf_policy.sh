...
if [ "$#" -lt 1 ]; then
  echo "usage: $0 {batch|group} [run_grpo_gemma.sh arguments...]" >&2
  exit 2
fi
...
case "${SCOPE}" in
  batch)
    SCOPE_FLAG="--use-dbc-self-inf-batch"
    ;;
  group)
    SCOPE_FLAG="--use-dbc-self-inf-group"
    ;;
  *)
    echo "error: scope must be batch or group; got '${SCOPE}'" >&2
    exit 2
    ;;
esac
...
LABEL="selfinf-${SCOPE}_policy__grpo_${RUN_TS}"
...
TUNIX_DBC_SELF_INF_POLICY=1 \
TUNIX_DBC_SELF_INF_LOO=0 \
TUNIX_DBC_SELF_INF_LOO_POLICY=0 \
TUNIX_DBC_SELF_INF_LOO_POLICY_ONLY=0 \
