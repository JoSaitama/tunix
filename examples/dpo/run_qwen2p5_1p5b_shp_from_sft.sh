...
if [[ "${FT_MODE}" == "lora" ]]; then
  CONFIG_PATH="${REPO_ROOT}/examples/dpo/qwen2p5_1p5b_shp_from_sft_lora.yaml"
else
  CONFIG_PATH="${REPO_ROOT}/examples/dpo/qwen2p5_1p5b_shp_from_sft.yaml"
fi
PYTHON_BIN="${PYTHON_BIN:-python3}"
export DPO_DATASET_MODULE="tunix/examples/data/shp_dpo.py:create_dataset"
export DPO_CLEAN_TEST_DATA_MODULE="examples/data/shp_dpo.py:create_dataset(split='test_prefs', partition='all', subset='all', seed=42)"
export DPO_RUN_DIR_PREFIX="dpo_qwen2p5_1p5b_shp_from_sft"
export DPO_RUN_DATASET_TAG="qwen2p5-1p5b-shp-from-sft"
