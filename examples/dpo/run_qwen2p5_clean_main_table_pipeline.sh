...
EVAL_SCRIPT="${REPO_ROOT}/examples/dpo/eval_qwen2p5_clean_main_table_runs.py"
SUMMARY_SCRIPT="${REPO_ROOT}/examples/dpo/summarize_qwen2p5_clean_main_table.py"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --output-root=*)
      ...
      ;;
    --benchmarks)
      BENCHMARKS="${2:?missing value for --benchmarks}"
      shift 2
      ;;
    --benchmarks=*)
      BENCHMARKS="${1#*=}"
...
if [[ -z "${OUTPUT_ROOT}" ]]; then
  OUTPUT_ROOT="${REPO_ROOT}/runs/results/qwen2p5_clean_main_table_${RUN_TS}"
fi
mkdir -p "${OUTPUT_ROOT}"
...
echo "Clean DPO main-table pipeline"
echo "  run_ts: ${RUN_TS}"
...
echo "  cleanup_exported_model: ${CLEANUP_EXPORTED_MODEL}"
echo
...
methods = [item for item in sys.argv[1].split(",") if item]
legacy_methods = [
    method for method in methods if method in table_lib.LEGACY_VARIANT_MAP
]
print(" ".join(legacy_methods))
PY
)"

if [[ "${NO_LEGACY}" -eq 0 && -n "${LEGACY_METHODS}" ]]; then
  python "${EVAL_SCRIPT}" \
    --run-ts "${RUN_TS}" \
    --legacy-run-ts "${LEGACY_RUN_TS}" \
    --profile "${PROFILE}" \
    --methods ${LEGACY_METHODS} \
    --seeds 0 \
    --benchmarks "${BENCHMARK_ARRAY[@]}" \
      --output-root "${OUTPUT_ROOT}" \
      $([[ "${FORCE_EVAL}" -eq 1 ]] && echo "--force")
fi

while IFS=$'\t' read -r STATUS SOURCE METHOD SEED RUN_ROOT COMMAND; do
  [[ -z "${STATUS}" ]] && continue
  PER_RUN_JSON="${OUTPUT_ROOT}/per_run/${METHOD}_seed${SEED}.json"
  echo "[${STATUS}] ${METHOD} seed=${SEED} (${SOURCE})"
  if [[ "${FORCE_EVAL}" -eq 0 && -f "${PER_RUN_JSON}" ]]; then
  ...
  if [[ "${SOURCE}" == "seeded" ]]; then
    echo "  eval: ${METHOD} seed=${SEED}"
    python "${EVAL_SCRIPT}" \
      --run-ts "${RUN_TS}" \
      --legacy-run-ts "${LEGACY_RUN_TS}" \
      --profile "${PROFILE}" \
      $([[ "${NO_LEGACY}" -eq 1 ]] && echo "--no-legacy") \
      --methods "${METHOD}" \
      --seeds "${SEED}" \
      --benchmarks "${BENCHMARK_ARRAY[@]}" \
      --output-root "${OUTPUT_ROOT}" \
      $([[ "${FORCE_EVAL}" -eq 1 ]] && echo "--force")
    if [[ "${CLEANUP_EXPORTED_MODEL}" -eq 1 ]]; then
...
python "${SUMMARY_SCRIPT}" \
  --run-ts "${RUN_TS}" \
  --input-json "${OUTPUT_ROOT}/per_run_metrics.json" \
